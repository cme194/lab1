...
//TODO: gather all the orig_data objects into one "data" object
data.pop_back();
...
distributed::bitonic_sort_binary( world, data);
}
...
for(auto i = 4; i < 9; ++i){
	t.restart();
	sort_problem( world, std::pow(2, i));
	double time = t.elapsed();
	double max_time = 0;
	//TODO: Gather all the running times here.
	if( world.rank() ==0){
